...
//! before passing it in (see [`apply_to_arc`] for the shared-Arc case).
...
struct ExtensionMatch {
    /// Parsed semver range from the selector's `@<range>` half.
    /// `None` when the selector is bare (`"is-positive"`) — applies
    /// to every version. An unparsable range silently falls through
    /// to "no match" instead of erroring, matching upstream's
    /// `semver.satisfies` which throws on invalid ranges. (Pacquet's
    /// node_semver returns `Err`; we treat that as "skip" so a typo'd
    /// range doesn't crash the whole install.)
    range: Option<Range>,
...
impl PackageExtender {
    ...
    /// input is empty.
    pub fn new(extensions: &IndexMap<String, PackageExtension>) -> Self {
        ...
        for (selector, extension) in extensions {
            ...
            let range =
                parsed.bare_specifier.as_deref().and_then(|range_str| range_str.parse().ok());
            by_pkg_name
                .entry(alias)
                .or_default()
                .push(ExtensionMatch { range, extension: extension.clone() });
        }
        PackageExtender { by_pkg_name }
    }
    ...
    pub fn apply(&self, manifest: &mut Value) {
        ...
        for entry in entries {
            if let Some(range) = entry.range.as_ref() {
                let Some(version) = version.as_ref() else { continue };
    ...
    pub fn apply_to_arc(&self, manifest: Arc<Value>) -> Arc<Value> {
        ...
        if !self.by_pkg_name.contains_key(name) {
            return manifest;
        }
