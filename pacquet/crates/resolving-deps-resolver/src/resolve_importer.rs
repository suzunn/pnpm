...
pub struct ResolveImporterOptions {
    ...
    /// the lockfile root are seeded into [`ParentRefs`] with a remapped
    /// node id (`link:<rel-from-lockfile_dir-to-modules_dir>/<alias>`)
    /// so the peer suffix stays stable across machines. Mirrors pnpm's
