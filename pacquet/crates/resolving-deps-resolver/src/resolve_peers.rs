...
pub struct ResolvePeersOptions {
    ...
    /// [`ParentRefs`] with a node id remapped to
...
impl<'tree> Walker<'tree> {
    ...
    ///    instead of carrying the raw link target. Mirrors upstream's
    ///    [`link:` arm](https://github.com/pnpm/pnpm/blob/094aa6e57b/installing/deps-resolver/src/resolvePeers.ts#L984-L989)
    ///    in `peerNodeIdToPeerId`. This branch fires for both
    ///    workspace-link parents and the `excludeLinksFromLockfile`
    ///    remap that points the parent at `link:node_modules/<alias>`.
    /// 2. The peer's `DepPath` once it has been walked — `node_dep_paths`
    ///    lookup, emitted as [`PeerId::DepPath`].
    /// 3. The cycle path: fall back to `name@version` from the resolved
    ///    package, emitted as [`PeerId::Pair`].
